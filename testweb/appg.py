...
## Model Generation
    ...
    df = df[start - 1:end - 1]
    correct = 0
    str = input('This is label', type=TEXT, placeholder='This is placeholder',
        help_text='This is help text', required=True)
    put_text(model.predict(vec.transform([str])))
    for i in range(len(df)):
        print(df.values[i][2])
        put_text(model.predict(vec.transform([df.values[i][2]])),df.values[i][9] == model.predict(vec.transform([df.values[i][2]])))
